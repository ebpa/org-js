...
Syntax.define("propertyDrawerStart", /^(\s*):PROPERTIES:\s*$/);
Syntax.define("propertyDrawerEntry", /^(\s*):(\S+):\s+(.*)\s*$/);
Syntax.define("propertyDrawerEnd", /^(\s*):END:\s*$/);
//Syntax.define("propertyDrawer", /^[ \t]*:PROPERTIES:[ \t]*\n\\(?:[ \t]*:\\S-+:\\(?: .*\\)?[ \t]*\n\\)*? [ \t]*:END:[ \t]*$/);

// TODO: Schedule/Deadline
//Syntax.define("planningLine", /^$/) org-maybe-keyword-time-regexp ///\\(\\<\\(\\(?:CLO\\(?:CK\\|SED\\)\\|DEADLINE\\|SCHEDULED\\):\\)\\)? *\\([[<][0-9]\\{4\\}-[0-9]\\{2\\}-[0-9]\\{2\\} ?[^]^M >]*?[]>]\\|<%%([^^M>]*>\\)/
...
Lexer.prototype = {
  tokenize: function (line) {
    ...
    if (Syntax.isHeader(line)) {
      ...
      token.indentation = RegExp.$1.length;
      token.content     = RegExp.$2;
    } else if (Syntax.isPropertyDrawerStart(line)) {
      token.type = Lexer.tokens.propertyDrawerStart;
      token.indentation = RegExp.$1.length;
      token.content = null;
    } else if (Syntax.isPropertyDrawerEntry(line)) {
      token.type = Lexer.tokens.propertyDrawerEntry;
      token.indentation = RegExp.$1.length;
      token.content = null;
      token.key = RegExp.$2;
      token.value = RegExp.$3;
    } else if (Syntax.isPropertyDrawerEnd(line)) {
      token.type = Lexer.tokens.propertyDrawerEnd;
      token.indentation = RegExp.$1.length;
      token.content = null;
