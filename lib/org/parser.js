...
Parser.prototype = {
  ...
  parseElement: function () {
    ...
    case Lexer.tokens.comment:
      ...
      this.lexer.getNextToken();
      break;
    case Lexer.tokens.propertyDrawerStart:
      this.parsePropertyDrawer();
      break;
    default:
  ...
  // ------------------------------------------------------------
  // <PropertyDrawer> ::= <PropertyDrawerStart> <PropertyDrawerEntry>* <PropertyDrawerEnd>
  // ------------------------------------------------------------
  parsePropertyDrawer: function () {
    if (this.lexer.peekNextToken().type === Lexer.tokens.propertyDrawerStart && this.lexer.getNextToken()) {

      if (!(this.nodes.length > 0) || this.nodes[this.nodes.length-1].type !== "header")
        throw this.createErrorReport("Property drawer must follow a heading");

      var heading = this.nodes[this.nodes.length-1];
      heading.properties = {};

      while (this.lexer.hasNext()) {
        var token = this.lexer.getNextToken();
        //console.log('token keys: '+JSON.stringify(Object.keys(token)));
        //console.log('token type: '+token.type);
        if (token.type === Lexer.tokens.propertyDrawerEntry) {
          if (token.key === "ID")
             this.document.ids[token.value] = heading;
          if (!(token.key in heading.properties)) {
            heading.properties[token.key] = token.value;
          } else if (heading.properties[token.key] instanceof Array) {
            heading.properties[token.key].push(token.value);
          } else {
            heading.properties[token.key] = [heading.properties[token.key], token.value];
          }
          continue;
        } else if (token.type === Lexer.tokens.propertyDrawerEnd) {
          return;
        }
        throw this.createErrorReport("Unclosed property drawer");
      }
    }
    throw this.createErrorReport("Did not find expected drawer token");
  }
};
